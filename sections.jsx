...
function Hero({ onJump, onPrefill, onJumpToProduct }) {
  return (
    <section id="top" className="relative dark-section-bg text-white overflow-hidden">
      <div className="max-w-[1360px] mx-auto px-6 pt-16 md:pt-24 pb-0 relative">
        <div className="grid lg:grid-cols-12 gap-10 lg:gap-12 items-center pb-16 md:pb-20 min-h-[72vh]">
          <div className="lg:col-span-7">
            <Reveal>
              <div className="inline-flex items-center gap-2 text-xs font-semibold tracking-wider text-accent uppercase mb-6">
                <span className="w-8 h-px bg-accent" />
                <span>Since 2012 · 경북 성주</span>
              </div>
            </Reveal>
            <Reveal delay={80}>
            ...
            <Reveal delay={180}>
              <p className="mt-8 max-w-xl text-white/75 text-[17px] md:text-[18px] leading-[1.8]">
                죠 크러셔 · 콘 크러셔 · 진동 스크린 · 피더를 <span className="text-white font-bold">자체 설계·제작</span>합니다.
                <span className="text-white"> 대표가 직접 상담합니다.</span>
              </p>
            </Reveal>
            ...
            <Reveal delay={260}>
              <div className="mt-10 flex flex-wrap gap-3">
                ...
                className="h-14 px-8 bg-accent text-white hover:brightness-110 shadow-lg hover:shadow-xl transition-all inline-flex items-center gap-3 rounded-sm">
                  <span className="font-bold">견적 문의하기</span>
                  <Icon name="ArrowRight" className="w-5 h-5" />
                </button>
                <a href={`tel:${COMPANY.tel.replace(/-/g, "")}`}
                className="h-14 px-8 border-2 border-white/80 text-white hover:bg-white hover:text-ink transition-colors inline-flex items-center gap-3 rounded-sm">
                  <Icon name="Phone" className="w-4 h-4" />
                  <span className="font-mono text-[14px] font-bold">{COMPANY.tel}</span>
                </a>
                <button onClick={() => onJump("equipment")}
                className="h-14 px-4 text-white/80 hover:text-accent transition-colors inline-flex items-center gap-2">
          ...
          <div className="lg:col-span-5">
            <Reveal delay={120}>
              <div className="relative">
                ...
                <div className="relative border border-white/15 bg-white/5 backdrop-blur-sm p-4 rounded-sm">
                  <BPCone sheetNo="DWG-HERO" className="aspect-[4/3] w-full" />
                  <div className="mt-3 flex items-start justify-between text-[11px] font-mono text-white/50">
                    <div>FIG.02 · CONE CRUSHER SECTION</div>
                    <div>— DSI</div>
                  </div>
                </div>
              </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 pb-16">
          ...
          className="text-left p-5 md:p-6 bg-white/5 backdrop-blur-sm border border-white/10 hover:bg-white/10 hover:border-accent/60 transition-all group rounded-sm">
              <div className="flex items-center justify-between">
                <div className="text-xs font-semibold text-white/40 tracking-wider">FIG · {n}</div>
                <Icon name="ArrowUpRight" className="w-4 h-4 text-white/40 group-hover:text-accent transition-colors" />
              </div>
              <div className="mt-2 text-[11px] font-bold tracking-wider text-accent uppercase">{cat}</div>
              <div className="mt-3 font-display font-bold text-2xl text-white group-hover:text-accent transition-colors">{en}</div>
              <div className="text-sm text-white/70 mt-0.5">{ko}</div>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="bg-accent text-white overflow-hidden border-t border-white/10">
        <div className="flex whitespace-nowrap marquee py-3.5 font-semibold text-[13px] tracking-[0.2em] uppercase">
          {Array.from({ length: 2 }).map((_, k) =>
          <span key={k} className="inline-flex items-center gap-10 px-6">
              <span>Daesuk Industrial</span><span className="text-white/60">◆</span>
              <span>Jaw · Cone · Screen · Feeder</span><span className="text-white/60">◆</span>
              <span>Turn-Key Delivery</span><span className="text-white/60">◆</span>
              <span>Genuine Wear Parts</span><span className="text-white/60">◆</span>
              <span>Since 2012</span><span className="text-white/60">◆</span>
              <span>Seongju · Korea</span><span className="text-white/60">◆</span>
            </span>
          )}
        </div>
      </div>
